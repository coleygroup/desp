...
from fwd_model import FwdTemplRel
...
def get_model(args, device) -> Tuple[nn.Module, Dict[str, Any]]:
    ...
    if args.load_from:
        ...
        model = FwdTemplRel(pretrain_args)
        ...
        if args.model_type == "templ_rel":
            args.output_dim = sum(
                1
                for _ in open(
                    os.path.join(args.processed_data_path, "v2/templates_v2.jsonl")
                )
            )
        elif args.model_type == "bb":
            args.output_dim = 256
        else:
            raise ValueError(f"Unsupported model type: {args.model}")
        model = FwdTemplRel(args)
